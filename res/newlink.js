requiredata.request('linktypedata', function(linktypedata) {
    requiredata.request('entityalias', function(alias) {
        ...
        linkableEntitiesPopup(getAlias({ alias: alias }, { alias: alias }, linktypedata.add[type]), undefined, function(selectedid) { 
            if (selectedid == undefined) { goBack(); return; } //if action canceled
            linkedid = selectedid; 
            ...
            var builder = $("#link-details").formbuilder({
                ...
                confirmExit: { enabled: true, msg: "Are you sure you want to exit without saving? New inserted data will be lost" }, cancel: goBack,
                save: { 
                    url: "./apis/links/edit.php", success: goBack, //redirects to previous page
